...
int activeEnemies = 0; // 현재 활성화된 적의 수
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT iMessage, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_PAINT:
	{
		...
		if (currentBitmapIndex == 2) { // 세 번째 맵에서만 적 그리기
		...
		if (showDirectionMessage) {
			ShowMessage(hDC, L"방향 조작 - WASD");
		}
		else if (showScanMessage) {
			ShowMessage(hDC, L"스캔 - E");
		}
		else if (showInteractionMessage) {
			ShowMessage(hDC, L"상호작용 - F");
		}
	...
	case WM_LBUTTONDOWN:
		if (currentBitmapIndex == 2) { // 세 번째 맵에서만 적을 클릭하여 제거 가능
	...
	case WM_TIMER:
		...
		if (wParam >= 100 && wParam < 120) { // 세 번째 맵의 적 타이머
			...
			if (enemiesX[index] < 0) {
				...
				if (removedEnemies < 10) { // 10마리의 적을 죽이지 못한 경우 게임 오버
					gameOver = true; // 게임 오버 상태 설정
					SetTimer(hWnd, gameOverTimerID, 3000, NULL); // 3초 후 게임 재시작
				}
			}
